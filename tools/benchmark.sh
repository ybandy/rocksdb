...
use_cxl_memory_for_block_cache=${USE_CXL:-0}
cxl_memory_latency=${CXL_LATENCY:-0}
cxl_set_latency_path=${CXL_SET_LATENCY_PATH:-""}
read_random_exp_range=${READ_RANDOM_EXP_RANGE:-"0.0"}
read_random_zipf_exponent=${READ_RANDOM_ZIPF_EXPONENT:-"0.0"}
...
const_params_base="
  ...
  \
  --num=$num_keys \
  --use_cxl_memory_for_block_cache=$use_cxl_memory_for_block_cache \
  --cxl_memory_latency=$cxl_memory_latency \
  --cxl_set_latency_path=$cxl_set_latency_path \
  --read_random_exp_range=$read_random_exp_range \
  --read_random_zipf_exponent=$read_random_zipf_exponent \
  --block_align=1 \
  --key_size=$key_size \
...
function run_filluniquerandom {
  ...
  echo "Compacting..."
  log_file_name=$output_dir/benchmark_filluniquerandom_compact.log
  time_cmd=$( get_cmd $log_file_name.time )
  cmd="$time_cmd ./db_bench --benchmarks=compact,stats \
       --use_existing_db=1 \
       --disable_auto_compactions=1 \
...
function run_readrandom {
  ...
  cmd="$time_cmd ./db_bench --benchmarks=readrandom,stats \
       ...
       --seed=$( date +%s ) \
       --report_file=${log_file_name}.r.csv \
       --checkpoint_trace_file=${log_file_name}.trace \
       --disable_auto_compactions=1 \
...
function run_readrandomwriterandom {
  echo "Reading and writing $num_keys random keys"
  log_file_name="${output_dir}/benchmark_readrandomwriterandom.t${num_threads}.log"
  time_cmd=$( get_cmd $log_file_name.time )
  cmd="$time_cmd ./db_bench --benchmarks=readrandomwriterandom,stats \
       --use_existing_db=1 \
       --readwritepercent=50 \
       $params_w \
  ...
  eval $cmd
  stop_stats $log_file_name.stats
  summarize_result $log_file_name readrandomwriterandom.t${num_threads} readrandomwriterandom
}
